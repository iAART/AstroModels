...
parser = argparse.ArgumentParser(description='Intensity Computation')
...
parser.add_argument('--tsnap', default=0, type=int)
...
if args.nnoisykey==1:
	...
	for tsnap in imagestimes:

		print("\n Computing an image at time t=%s (M) \n"%np.round(tsnap,5))

		t012 = [ np.mod(t0rt+fact2+tsnap,xtend), np.mod(t1rt+fact2+tsnap,xtend), np.mod(t2rt+fact2+tsnap,xtend)]

		obsint.br(supergrid0,mask0,N0,rs0,sign0,anglen0,supergrid1,mask1,N1,rs1,sign1,anglen1, supergrid2,mask2,N2,rs2,sign2,anglen2,brightparams,funckeys,phi012,t012,interpolated3_R,tsnap)

else:

	h5f.close()

	obsint.br_r(supergrid0,mask0,N0,rs0,sign0,anglen0,supergrid1,mask1,N1,rs1,sign1,anglen1,
			  supergrid2,mask2,N2,rs2,sign2,anglen2,brightparams,funckeys)
